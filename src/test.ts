...
const url = () => {
  if (!PROCA_USERNAME || !PROCA_PASSWORD) {
    throw new Error("Either PROCA_URL or both PROCA_USERNAME and PROCA_PASSWORD must be set");
  }

  return `amqps://${PROCA_USERNAME}:${encodeURIComponent(PROCA_PASSWORD)}@api.proca.app/proca_live`;
};
...
syncQueue(url(), PROCA_QUEUE as string, handler, {
  concurrency: 1,
  tag: "local-dev",
